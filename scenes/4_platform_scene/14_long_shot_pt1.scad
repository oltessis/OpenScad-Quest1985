...
include <decorative_tree2_basin_pine.scad>
include <decorative_tree3_cheesehole.scad>
include <decorative_tree3_cheesehole_lowres.scad>
include <decorative_tree1_snumb.scad>
include <decorative_tree4.scad>
include <decorative_tree4_lowres.scad>
include <temple_water_basin.scad>
include <helicopter.scad>
include <helicopter_transform_animation.scad>
//include <helicopter_top_no_head.scad>
include <helicopter_top_no_head_animated.scad>
//include <helicopter_to_bike1.scad>
//include <helicopter_to_bike2.scad>
//include <helicopter_to_bike3.scad>
include <helicopter_to_bike4.scad>
include <bike_animation.scad>


$vpt = [35.57+(178.89*$t)+60, -84.68+(88.04*$t)-20, 19.48+(1*$t)];   // camera position
$vpr = [76.7+(14.7*$t), 0, 337.6+(97.3*$t)];    // rotation
$vpd = 40-(20*$t);                  // distance from camera
$vpf = 50-(36*$t);                   // field of view (perspective only)
 

/*
// very small camera distance
$vpd = 18 - (2*$t);

// compensate by pushing camera position forward
$vpt = [
    35.57 + (178.89*9.2*$t),
   -84.68 + ( 88.04*9.2*$t),
    19.48 + (  1.00*9.2*$t)
...
// keep rotation the same (important!)
$vpr = [
    76.7  + (14.7*$t),
    0,
    337.6 + (97.3*$t)
...
// tighten FOV less aggressively since distance is already small
$vpf = 38 - (18*$t);
*/
//$vpf = 25;

translate([52+40+110, -300+50+130+20, 2.5+1])
scale(0.8)
rotate([0,0,90+45])
translate([100*$t,0,0])
bike_animation();
...
translate([135,correction_factor+65,5])
scale(1.2)
sirpinski_structure();
translate([-135,correction_factor+65,5])
scale(1.2)
...
rotate([0,180,0])
for (i = [-55:27:55]) {
 translate([i,42,24-correction_factor])
 rotate([-30,0,0])
 scale(1.1)
 front_temple_column();   
}
