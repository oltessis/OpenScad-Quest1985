...
circleThickness = 1;
radius = 3.8;
excess_rotation = -36-(80*$t);

module heli_to_bike(head_rotation) {
    ...
    rotate([12-(40*$t),0,90+i+(360)+excess_rotation])
    translate([0,7.5,0])
    ...
    // black smarties
    for (i = [0:120:360]) {
    translate([0,0,14-(13*$t)])
    rotate([0,12-(12*$t),i+60+(360)+excess_rotation])  
    translate([13.5,0,0])
    color("Black")
    scale([1,1,0.15])
    sphere(radius,$fn=40);
    ...
    // head
    translate([0,0,16])
    rotate([180-(90),90,head_rotation])
    union(){
