...
module heli_to_bike() {
    ...
    color("DarkGray")
    translate([0,0,radius*2-(7)])
    rotate([-28+(128)-(10*$t),0,90+i+(360)+excess_rotation])
    translate([0,6,0-(3)+(2.9*$t)])
    ...
    // black smarties
    for (i = [0:120:360]) {
    translate([(-1)+(1*$t),(-1)+(1*$t),1+(14)])
    rotate([0,12-(12),i+60+(360)+excess_rotation])  
    translate([13.5-(11)-(2.5*$t),0,-4*$t])
    color("Black")
    scale([1,1,0.15])
    sphere(radius,$fn=40);
    }
