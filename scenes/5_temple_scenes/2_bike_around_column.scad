...
$vpt = [0.83, 119.20, 35.23];   // camera position
$vpr = [96.3, 0.00, 2.6];    // rotation
$vpd = 203.13;                  // distance from camera
$fov = 22.50;                   // field of view (perspective only)


translate([6*(1-cos(90*pow($t,0.25))),90+(19*2)+(8*sin(90*pow($t,3))),21])
rotate([0+(15*(1-cos(360*$t))/2),-30+(30*pow($t,0.1)),90-(20*(1-cos(360*$t)))])
scale(0.48)
bike_animation();

translate([6*(1-cos(90*pow($t,0.25))),91.6+(19*2)++(8*sin(90*pow($t,3))),18])
rotate([180-(15*(1-cos(360*$t))/2),-30+(30*pow($t,0.1)),90-(20*(1-cos(360*$t)))])
scale(0.48)
bike_animation();
