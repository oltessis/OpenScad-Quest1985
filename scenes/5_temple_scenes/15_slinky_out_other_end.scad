...
$vpt = [-3.42, 483, 32.47];   // camera position
$vpr = [89.2, 0.00, 88.7];    // rotation
$vpd = 29.7;                  // distance from camera
$vpf = 20;                   // field of view (perspective only)
 


// slinky's might be sitting one unit too low
translate([15,478, 46+(3*$t)])
rotate([0, -38+(90*$t), -90])
scale(0.48)
slinky();
...
translate([-41.5,511.5,18.8])
rotate([0,0,200])
translate([0,32,0.3])
rotate([0,0,70])
scale(2.9)
temple_sanctum_column();
translate([-63.5,528.5,21])
rotate([0,0,200])
translate([0,32,0.3])
rotate([0,0,70])
scale(2.5)
...
// reflection
rotate([0,180,0])
translate([0,462+correction_factor,-17])
difference()
{
union(){
scale(2.5)
