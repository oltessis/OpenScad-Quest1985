...
$vpt = [-37.39, 473, 31.05];   // camera position
$vpr = [86.5, 0.00, 89];    // rotation
$vpd = 55;                  // distance from camera
$vpf = 25;                   // field of view (perspective only)


//$vpf = 40;

// slinky's might be sitting one unit too low
translate([7,465+(13*$t), 29.4])
translate([0,0,5])
rotate([180, 0, -90])
scale(0.48)
slinky();

translate([-8+(8),395+(47)+(24), 19.8-(4)])
rotate([0, 0, -90])
scale(0.48)
slinky();
...
// temple hall 
translate([0,correction_factor+1.5,0])
union(){
// normal
for (i = [150:14:320]) {
    
translate([-15,60+i,33])
rotate([0,0,90])
scale([1.8,1.8,1.8])
temple_column();
...
radia_subtract = 300;
rotation_correction = 22.5*5;

translate([0,462+correction_factor+1.5,20])
difference()
{
union(){
scale(2.5)
for (i = [0:22.5:360-radia_subtract]) {
 rotate([0,0,i+rotation_correction])
 translate([0,32,0])
 rotate([0,0,22])
