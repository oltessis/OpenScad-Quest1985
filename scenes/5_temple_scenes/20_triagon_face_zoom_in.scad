...
$vpt = [0, 555, 36-(7*$t)];   // camera position
$vpr = [90+(6*$t), 0.00, 180];    // rotation
$vpd = 10;                  // distance from camera
$vpf = 25-(20.5*$t);                   // field of view (perspective only)
...
translate([0,478, -10+(3*$t)])
rotate([180, -38+(90*$t), -90])
scale(0.48)
slinky();
...
for (i = [0:22.5:(360-25)]) {
 ...
 rotate([0,0,i+12.25+col_correction])
 translate([0,34,2.5])
 scale(0.6)
