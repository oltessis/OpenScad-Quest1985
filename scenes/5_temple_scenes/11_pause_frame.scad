...
include <slinky_neck_animation.scad>
include <triagon3d.scad>

// 15-20 columns deep


$vpt = [-29.81, 409.16+(40)+(48)+(47), 39.07];   // camera position
$vpr = [90, 0.00, 15.9];    // rotation
$vpd = 140.3;                  // distance from camera
$vpf = 45;                   // field of view (perspective only)


// slinky's might be sitting one unit too low
translate([0-(6*$t),395+(47)+(24)-(48*$t), 29.7+(4)-(2.8*$t)])
rotate([0,790*$t,0])
translate([0,0,5*$t])
rotate([180+(180*$t), 46*$t, -90])
scale(0.48)
*slinky();
