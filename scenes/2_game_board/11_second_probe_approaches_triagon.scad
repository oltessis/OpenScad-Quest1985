...
// probe ring 
pivot_point = [0,-31,27]; 
rotate_index = 450 + (12*$t); // 15 deg per cycle
rotate_step = 22.5;
probe1_factor = 5;
probe_scale = 0.6;
probe_distance = 57;
...
translate(pivot_point)
rotate([0,0,630])
translate([probe_distance-(40*$t),0,0])
scale(probe_scale)
rotate([0,0,90])
_2dprobe2();
...
$vpt = [19.06, -142.62, 56.42];   // camera position
$vpr = [80.20, 0.00, 169.50];    // rotation
$vpd = 208.39;                  // distance from camera
