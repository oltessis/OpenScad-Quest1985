...
// probe ring 
pivot_point = [0,-31,27]; 
rotate_index = 462; // 15 deg per cycle
rotate_step = 22.5;
probe1_factor = 5;
probe_scale = 0.6;
probe_distance = 57;
...
// number 8 
// active probe for 2nd
translate(pivot_point)
rotate([0,0,630])
translate([probe_distance-49-(1.5*$t),0,0])
scale(probe_scale)
rotate([0,0,90])
_2dprobe2();

rotate([0,0,rotate_index+(rotate_step*7)+probe1_factor])
translate([36.5,3.5,27])
color("white",0.3+(0.6*$t))
sphere(6+(4.5*$t),$fn=100);
...
$vpt = [0, -145, 55.72];   // camera position
$vpr = [0.00, 0.00, 0.00];    // rotation
$vpd = 34;                  // distance from camera
